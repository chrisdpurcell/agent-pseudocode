...
KEYWORD_COMPLETIONS: tuple[CompletionSpec, ...] = (
    CompletionSpec(
        "process",
        'process ${1:name}(${2:inputs}):\n    ${3:state} = ${4:initialize_state}(${2:inputs})\n    return ${5:Accepted}(reason="${6:complete}")',
        "workflow declaration",
        "Defines an agent workflow. This is a convention extension, not executable Python.",
        KIND_SNIPPET,
        True,
    ),
    CompletionSpec(
        ...
        "if ${1:condition}:\n    ${2:action}()",
        "branch",
        "Starts a conditional branch. Prefer an explicit `else` unless the branch set is annotated exhaustive.",
        KIND_SNIPPET,
        True,
    ),
    CompletionSpec(
        ...
        "elif ${1:condition}:\n    ${2:action}()",
        "branch",
        "Adds a mutually exclusive branch to an `if` chain.",
        KIND_SNIPPET,
        True,
    ),
    CompletionSpec(
        "else",
        'else:\n    return Blocked(reason="${1:unhandled case}")',
        "fallback branch",
        "Provides an explicit fallback branch.",
        KIND_SNIPPET,
        True,
    ),
    CompletionSpec(
        ...
        "bounded loop",
        "Repeats while a bounded condition holds. Every `while` loop must have a cap, timeout, or explicit external stop condition.",
        KIND_SNIPPET,
        True,
    ),
    CompletionSpec(
        ...
        "bounded iteration",
        "Iterates over a finite collection or bounded range.",
        KIND_SNIPPET,
        True,
    ),
    CompletionSpec(
        "return",
        'return ${1:Accepted}(reason="${2:complete}")',
        "terminal outcome",
        "Exits the process immediately with an approved outcome.",
        KIND_SNIPPET,
        True,
    ),
    CompletionSpec(
        "continue", "continue", "loop control", "Continues the current loop.", KIND_KEYWORD
    ),
...
NORMATIVE_COMPLETIONS: tuple[CompletionSpec, ...] = (
    CompletionSpec(
        ...
        "MUST",
        "normative keyword",
        "Absolute requirement in this convention.",
        KIND_KEYWORD,
    ),
    CompletionSpec(
        "MUST NOT",
        "MUST NOT",
        "normative keyword",
        "Absolute prohibition in this convention.",
        KIND_KEYWORD,
    ),
    CompletionSpec(
        "SHOULD",
        "SHOULD",
        "normative keyword",
        "Recommended behavior unless there is a documented reason to differ.",
        KIND_KEYWORD,
    ),
    CompletionSpec(
        "SHOULD NOT",
        "SHOULD NOT",
        "normative keyword",
        "Discouraged behavior unless there is a documented reason.",
        KIND_KEYWORD,
    ),
    CompletionSpec("MAY", "MAY", "normative keyword", "Optional behavior.", KIND_KEYWORD),
    CompletionSpec(
        "REQUIRED", "REQUIRED", "normative keyword", "Required behavior or artifact.", KIND_KEYWORD
    ),
    CompletionSpec(
        "OPTIONAL", "OPTIONAL", "normative keyword", "Optional behavior or artifact.", KIND_KEYWORD
    ),
)
...
ANNOTATION_COMPLETIONS: tuple[CompletionSpec, ...] = (
    CompletionSpec(
        ...
        "@bounded",
        "loop annotation",
        "Marks a loop as intentionally bounded when the bound is not obvious syntactically.",
        KIND_ENUM_MEMBER,
    ),
    CompletionSpec(
        ...
        "@external_stop_condition",
        "loop annotation",
        "Marks a loop as governed by an external stop condition.",
        KIND_ENUM_MEMBER,
    ),
    CompletionSpec(
        ...
        "@explicit_stop_condition",
        "loop annotation",
        "Documents that the adjacent loop has an explicit stop condition.",
        KIND_ENUM_MEMBER,
    ),
    CompletionSpec(
        "@timeout",
        "@timeout",
        "loop annotation",
        "Documents a timeout-based stop condition.",
        KIND_ENUM_MEMBER,
    ),
    CompletionSpec(
        "@round_cap",
        "@round_cap",
        "loop annotation",
        "Documents a review/revision round cap.",
        KIND_ENUM_MEMBER,
    ),
    CompletionSpec(
        "@finite_collection",
        "@finite_collection",
        "for-loop annotation",
        "Marks an iterable source as finite even when the expression is not obvious.",
        KIND_ENUM_MEMBER,
    ),
    CompletionSpec(
        "@exhaustive",
        "@exhaustive",
        "branch annotation",
        "Marks an if/elif chain as exhaustive so an else fallback is not required.",
        KIND_ENUM_MEMBER,
    ),
    CompletionSpec(
        "@allow_empty_branch",
        "@allow_empty_branch",
        "branch annotation",
        "Allows a placeholder branch body intentionally.",
        KIND_ENUM_MEMBER,
    ),
    CompletionSpec(
        "@verified",
        "@verified",
        "action annotation",
        "Marks a mutating action as verified when the verification is not visible nearby.",
        KIND_ENUM_MEMBER,
    ),
    CompletionSpec(
        ...
        "@no_verification_required",
        "action annotation",
        "Documents why a mutating action does not need a verification step.",
        KIND_ENUM_MEMBER,
    ),
    CompletionSpec(
        ...
        "@allow_missing_process",
        "snippet annotation",
        "Allows a snippet without a top-level process declaration.",
        KIND_ENUM_MEMBER,
    ),
)
...
ACTION_COMPLETIONS: tuple[CompletionSpec, ...] = (
    CompletionSpec(
        "review_document",
        "review_document(${1:document})",
        "agent action",
        "Review an artifact and return review status/blockers.",
        KIND_FUNCTION,
        True,
    ),
    CompletionSpec(
        "revise_using_blockers",
        "revise_using_blockers(${1:document}, ${2:blockers})",
        "agent action",
        "Revise an artifact using unresolved blockers.",
        KIND_FUNCTION,
        True,
    ),
    CompletionSpec(
        ...
        "run_tests()",
        "verification action",
        "Run the relevant test suite or verification check.",
        KIND_FUNCTION,
        True,
    ),
    CompletionSpec(
        "validate_output",
        "validate_output(${1:artifact})",
        "verification action",
        "Validate an output artifact against the acceptance checks.",
        KIND_FUNCTION,
        True,
    ),
    CompletionSpec(
        ...
        "commit_changes()",
        "mutating action",
        "Commit or persist a completed change. Follow with verification or a terminal outcome.",
        KIND_FUNCTION,
        True,
    ),
)
...
SNIPPET_COMPLETIONS: tuple[CompletionSpec, ...] = (
    CompletionSpec(
        "review-loop",
        'process ${1:review_loop}(${2:document}, max_rounds=${3:5}):\n    round = 1\n    blockers = []\n\n    while round <= max_rounds:\n        review = review_document(${2:document})\n\n        if review.status == "approved":\n            return Accepted(reason="approved")\n\n        elif review.blockers:\n            blockers = review.blockers\n            ${2:document} = revise_using_blockers(${2:document}, blockers)\n            round += 1\n            continue\n\n        else:\n            return Blocked(reason="ambiguous review result")\n\n    return Blocked(reason="round cap reached", unresolved_blockers=blockers)',
        "workflow snippet",
        "Bounded review/revision loop with explicit success and failure outcomes.",
        KIND_SNIPPET,
        True,
    ),
    CompletionSpec(
        "guard-block",
        'if ${1:missing_required_input}:\n    return Blocked(reason="${2:missing required input}")',
        "guard clause snippet",
        "Early exit for missing required input or invalid preconditions.",
        KIND_SNIPPET,
        True,
    ),
    CompletionSpec(
        "decision-fallback",
        'else:\n    return Blocked(reason="${1:unhandled decision branch}")',
        "fallback snippet",
        "Explicit fallback for non-exhaustive branch chains.",
        KIND_SNIPPET,
        True,
    ),
)
